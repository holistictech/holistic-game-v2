...
public class KsilofonSpan : MonoBehaviour
{
    ...
    private IEnumerator ReadyForStart() {
        ...
        var messages = new[] {
            new { Text = "Ksilefon Span'e hoşgeldiniz!", Time = 1f },
    ...
    private IEnumerator PlaySound() {
        ...
        Button newButton = buttons[Random.Range(0, buttons.Length)];
    ...
    private IEnumerator StartPlayerTurn() {
        ...
        if (level == 1) {
            countdownDisplay.text = "Hangi nota çaldı?";
            yield return new WaitForSeconds(1f);
        } else {
            countdownDisplay.text = "Hangi nota eklendi?";
        }
    ...
    public void EnteredOption(Button button)
    {
        ...
        if (isGameActive) {
            ...
            if (playedSoundNames[currentIndex - 1] != button.name) {
                // If not, the user made a mistake. Call checkSuccess with false
                checkSuccess(false);
            } else if (currentIndex == playedButtons.Count) {
                // If the user has entered all options correctly, check if the entire sequence is correct
