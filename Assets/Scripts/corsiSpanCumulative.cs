...
public class corsiSpanCumulative : MonoBehaviour
{
    ...
    private IEnumerator StartStep(int showingBlockCount)
    {
        ...
        for (int i = 0; i < showingBlockCount; i++)
        {
            ...
            if (startingBlockCount < 4)
            {
                CreateElementsInDigitCircle(shownIndices.Count);
            }
            ...
            {
                int lastIndex = shownIndices.Count - 1;
                GameObject newImage = Digitcircle.transform.GetChild(lastIndex).gameObject;
                ChangeImageColor(newImage, Color.green);
            }
    ...
    public void ChoosedBlocks(Button block)
    {
        ...
        if (isCheckingInput == true)
        {
            ...
            else if (block.name == "CheckAnswer")
            {
                ...
                blocks.SetActive(false);
                bool isSuccess = IsListEqual(enteredBlocks, shownIndices);
            ...
            else
            {
                ...
                if (currentIndex == shownIndices.Count)
                {
    ...
    private IEnumerator LevelTimeStart()
    {
        ...
        levelTimeCountdown = (shownIndices.Count * 3) + 2;
