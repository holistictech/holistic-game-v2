    ...
    public void TurnCard()
    {
        ...
        if (imageComponent != null)
        {
            if (IsFlipped)
            {
                if (SoundClip != null)
                {
                    AudioSource.PlayClipAtPoint(SoundClip, Vector3.zero);

                    Sprite speakerImage = Resources.Load<Sprite>("speaker");
                    imageComponent.sprite = speakerImage;
                }
                else
                {
...
public class MemoryCardAudio : MonoBehaviour
{
    private List<AudioCard> cards = new List<AudioCard>();
    private List<AudioCard> matchedCards;
    private AudioCard previousCard;
    ...
    private List<AudioClip> generatedAudioClips = new List<AudioClip>();
    private List<string> generatedImageNames = new List<string>();
    ...
    private AudioCard firstRevealed = null;
    private AudioCard secondRevealed = null;
    ...
    private IEnumerator CountdownToStart()
    {
        ...
        countdownDisplay.text = "MemoryCardAudio oyununa hoşgeldiniz!";
    ...
    public void CardRevealed(AudioCard card)
    {
    ...
    {
        if (firstRevealed.CardObject.name == secondRevealed.CardObject.name)
        {
            Debug.Log("Matched");
    ...
    private IEnumerator AssignImagesToCards()
    {
        ...
        allImages = Resources.LoadAll<Sprite>("Animals").ToList();
        ...
        foreach (Sprite image in generatedImages)
        {
            generatedImageNames.Add(image.name);
        }

        foreach (string imageName in generatedImageNames)
        {
            AudioClip audioClip = Resources.Load<AudioClip>("Sounds/" + imageName);
            generatedAudioClips.Add(audioClip);
        }

        ShuffleList(generatedImageNames);
        ShuffleList(generatedAudioClips);

        maxMoves = (int)(generatedImages.Count * 3);
    ...
    private IEnumerator ReadyToStart()
    {
        ...
        countdownDisplay.text = "Resimleri seslerle eşleştirelim!";
        yield return new WaitForSeconds(1.0f);
        countdownDisplay.gameObject.SetActive(false);
    ...
    private void CreateOptions(int numberOfElements)
    {
        ...
        {
            Sprite randomImage = generatedImages[i];
        ...
        for (int j = 0; j < generatedAudioClips.Count; j++)
        {
            AudioClip soundClip = generatedAudioClips[j];

            Sprite speakerImage = Resources.Load<Sprite>("speaker");

            GameObject cardObject = Instantiate(optionPrefab, options.transform);
            cardObject.transform.localPosition = new Vector3(j * 2, 0, 0);
            cardObject.GetComponent<Image>().sprite = speakerImage;
            cardObject.name = soundClip.name;
    ...
    private void GameStartRules()
    {
        ...
        generatedImageNames.Clear();
        generatedAudioClips.Clear();
        moves = 0;
        maxMoves = 3;
    }
    ...
    {
        levelTimeCountdown = (generatedImages.Count * 8);
