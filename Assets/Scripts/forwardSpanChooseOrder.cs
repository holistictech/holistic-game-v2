...
public class forwardSpanChooseOrder : MonoBehaviour
{
    ...
    private IEnumerator CountdownToStart()
    {
        ...
        StartCoroutine(LoadBlocks(startingBlockCount));

    }

    private IEnumerator LoadBlocks(int count)
    {
        yield return new WaitForSeconds(1);
        ...
        StartCoroutine(StartStep(count));
    }
    ...
    IEnumerator HideSuccessDialog(float delay, int count)
    {
        yield return new WaitForSeconds(delay);
        succedDialog.SetActive(false);
        blocks.SetActive(true);
        yield return new WaitForSeconds(1.5f);
        StartCoroutine(LoadBlocks(count));
